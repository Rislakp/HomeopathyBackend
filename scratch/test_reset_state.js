...
function generateAnswerKeyPDFInline(exam, user) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Helper: Draw Watermark on current page WITHOUT ROTATION TEST
      function drawWatermarkOnCurrentPage() {
        ...
        doc.text(watermarkText, MARGIN, 400, {
          width: CONTENT_WIDTH,
          align: 'center',
          lineBreak: false
        });
        ...
        doc.font('Helvetica').fontSize(10).fillColor('#1A202C').opacity(1.0);
      }
...
async function testResetState() {
  console.log('--- TESTING EXPLICIT STATE RESET ARCHITECTURE ---');
  ...
  console.log('\n🎉 ALL EXPLICIT STATE RESET TESTS PASSED PERFECTLY!');
}

testResetState().catch(console.error);
